...
@ActivityScope
public class VerifyIdCardPresenter extends BasePresenter<VerifyIdCardContract.Model, VerifyIdCardContract.View> {
    ...
    @Inject
    public VerifyIdCardPresenter(VerifyIdCardContract.Model model, VerifyIdCardContract.View rootView
                    ...
                    public void onNext(BaseData<AirportGoInfoRequest> airportGoInfoRequestBaseData) {
                        if(airportGoInfoRequestBaseData.isSuccess()) {
                            if(airportGoInfoRequestBaseData.getData() != null) {
                                mRootView.setAirportGoInfoRequest(airportGoInfoRequestBaseData.getData());
                            }else{
                                //数据为空
                            }
                        }else{//失败
    ...
    public void getHasBookInfo(AirportGoInfoRequest params){
        mModel.getHasBookInfo(params)
                .compose(RxUtils.<BaseData<AirportGoInfoRequest>>applySchedulers(mRootView))
                .subscribe(new ErrorHandleSubscriber<BaseData<AirportGoInfoRequest>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseData<AirportGoInfoRequest> airportGoInfoRequestBaseData) {
                        if(airportGoInfoRequestBaseData.isSuccess()) {
                            if(airportGoInfoRequestBaseData.getData() != null) {
                                mRootView.setHasBookInfo(airportGoInfoRequestBaseData.getData());
                            }else{
                                //数据为空
                            }
                        }else{//失败
