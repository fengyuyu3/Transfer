...
*/
    ...
    public void moveLooper(LatLng endPoint) {
        ...
        mMoveMarker.setRotate((float) CommonBaiDuUtil.getAngle(lastPoint, endPoint));

        double slope = CommonBaiDuUtil.getSlope(lastPoint, endPoint);
        // 是不是正向的标示（向上设为正向）
        boolean isReverse = (lastPoint.latitude > endPoint.latitude);
        double intercept = CommonBaiDuUtil.getInterception(slope, lastPoint);
        double xMoveDistance = isReverse ? CommonBaiDuUtil.getXMoveDistance(slope) : -1 * CommonBaiDuUtil.getXMoveDistance(slope);
    ...
    public void drawHistoryTrack(List<LatLng> points, SortType sortType, String arrivedTime) {
        ...
        OverlayOptions markerOptions =
                new MarkerOptions().flat(true).anchor(0.5f, 0.5f).icon(bmArrowPoint)
                        ...
                        .rotate((float) CommonBaiDuUtil.getAngle(points.get(0), points.get(1)));
